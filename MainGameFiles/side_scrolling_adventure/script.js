  ...
  class Player {
    ...
    update(input, deltaTime, enemies) {
      ...
      enemies.forEach((enemy) => {
        ...
        const dy =
          enemy.y + enemy.height / 4 - (this.y + this.height * HIT_BOX_HEIGHT); // and from bottom of player to middle of player
  ...
  function displayStatusText(context) {
    ...
    if (gameOver) {
      ...
      canvas.addEventListener("click", function (event) {
        ...
        if (mouseX >= 283 && mouseX <= 523 && mouseY >= 315 && mouseY <= 385) {
          location.reload();
        }
        if (mouseX >= 223 && mouseX <= 583 && mouseY >= 415 && mouseY <= 485) {
          goBackToHome();
